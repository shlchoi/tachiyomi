...
class MangaPresenter(
    ...
    private fun applyChapterFilters(chapters: List<ChapterItem>): Observable<List<ChapterItem>> {
        ...
        val hiddenFilter = showHidden()
        if (hiddenFilter == State.INCLUDE) {
            observable = observable.filter { it.isHiddenInList }
        } else if (hiddenFilter == State.EXCLUDE) {
            observable = observable.filter { !it.isHiddenInList }
        }
    ...
    fun setHiddenFilter(state: State) {
        manga.showHidden = when (state) {
            State.IGNORE -> Manga.SHOW_ALL
            State.INCLUDE -> Manga.SHOW_HIDDEN
            State.EXCLUDE -> Manga.HIDE_HIDDEN
        }
        db.updateChapterFlags(manga).executeAsBlocking()
        refreshChapters()
    }
    ...
    fun showHidden(): State {
        return when (manga.showHidden) {
            Manga.SHOW_HIDDEN -> State.INCLUDE
            Manga.HIDE_HIDDEN -> State.EXCLUDE
            else -> State.IGNORE
        }
    }
